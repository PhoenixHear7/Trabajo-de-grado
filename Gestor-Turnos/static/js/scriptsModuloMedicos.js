  ...
  async function aceptarTurno(turno) {
    try {
      ...
      const veterinarioOcupado = turnos.some(
        (t) =>
          t.estado === "proceso" &&
          t.veterinario_id === turno.veterinario_id &&
          t.id !== turno.id
      );
      ...
      // Verificar si el consultorio donde se va a atender ya tiene un turno en proceso
      const consultorioOcupado = turnos.some(
        (t) =>
          t.estado === "proceso" &&
          t.modulo === consultorioId &&
          t.id !== turno.id
      );

      if (consultorioOcupado) {
        alert(
          "Este consultorio ya tiene un turno en proceso. Solo puede haber un turno en proceso por consultorio."
        );
        return;
      }
